...
class Channel():
	...
	def join(self, subgroup):
		...
		created = False
		newpid:int = -1 #If a node with pid=-1 is created something is wrong
		while not created:
			...
			if(IDLock == "True"):
				...
				newpid = random.choice(list(set([str(i) for i in range(self.MAXPROC)]) - members)) # type:ignore
				if len(members) > 0:
					xchan = []
	...
	# These functions might be useful in future
